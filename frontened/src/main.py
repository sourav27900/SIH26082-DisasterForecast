...
def generate_delhi_ncr_stations():
    ...
    stations = {
        "name": [
            "Anand Vihar", "Connaught Place", "Gurugram Cyber City", 
            "Noida Sector 62", "Okhla Phase 3", "Punjabi Bagh",
            "IIT Delhi", "Dwarka", "Rohini", "Ghaziabad"
        ],
        "latitude": [28.6508, 28.6280, 28.4947, 28.6200, 28.5300, 
                     28.6630, 28.5921, 28.5921, 28.7041, 28.8694],
        "longitude": [77.3152, 77.2090, 77.0880, 77.3640, 77.2800, 
                      77.1250, 77.1925, 77.0452, 77.0852, 77.5580],
...
def generate_thermal_stress_index():
    ...
    data["Thermal_Stress_Index"] = (
        data["temperature"] + 0.5 * (data["humidity"] / 100 * data["temperature"])
    )
...
theme_indicator = "🌙 Dark Mode Active" if theme_mode == "🌙 Dark Mode" else "☀️ Light Mode Active"
st.caption(f"{theme_indicator} | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
st.markdown("---")
...
with tab1:
    st.subheader("Live Thermal Stress & Pollution Hotspot Map")
    st.caption("Hover over nodes to check station-specific micro-climate data. Node size represents temperature intensity.")
    ...
    st.dataframe(
        data[["name", "temperature", "aqi", "pm25", "thermal_stress"]].sort_values("aqi", ascending=False),
...
with tab2:
    ...
    with col2:
        ...
        fig_aqi.add_hline(y=200, line_dash="dash", line_color="orange", annotation_text="Moderate", annotation_position="right")
        fig_aqi.add_hline(y=300, line_dash="dash", line_color="red", annotation_text="Poor", annotation_position="right")
...
with tab3:
    ...
    with col1:
        ...
        fig_7day_temp.add_trace(go.Bar(
...
with tab4:
    ...
    thermal_index_sorted = thermal_index_data.sort_values("Thermal_Stress_Index", ascending=False)
    ...
    st.dataframe(
        thermal_index_sorted[[
            "name", "temperature", "humidity", "wind_speed", "Thermal_Stress_Index", "thermal_stress"
...
with tab5:
    ...
    severe_count = len(thermal_index_data[thermal_index_data["thermal_stress"] == "Severe Heatwave"])
    moderate_count = len(thermal_index_data[thermal_index_data["thermal_stress"] == "Moderate Heatwave"])
    ...
    st.markdown(f"<h3 style='color:{alert_color}'>{alert_level} ACTIVE</h3>", unsafe_allow_html=True)
